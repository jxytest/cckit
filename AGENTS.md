...
`clab` 是一个 **pip 可安装的多 Agent SDK**，基于 `claude-agent-sdk`（封装 Claude CLI 的 Python SDK）构建。提供：

- **声明式 Agent 定义** — `Agent()` 实例化即定义，无需继承
- **LiteLLM 模型桥接** — 通过 `LiteLlm()` 支持 100+ 模型提供商
...
平台 MCP 工具在 `clab/tools/platform.py`，用户显式导入：
...
继承 `Middleware`，实现 `async def wrap(self, next_call, prompt, options, collector, ctx)`：
