...
# 基础用法（向后兼容）
async for message in runner.run_stream(assistant, ctx):
    print(message)
...
| **Runner** | `Runner()` | "执行引擎" — default_model、workspace_root、中间件、并发控制、SDK 桥接 |
...
from cckit import Agent, ModelConfig, Runner, RunnerConfig, SandboxOptions
...
# 方式 2: 从环境变量读取
config = RunnerConfig.from_env()  # 读取 ANTHROPIC_*、PLATFORM_*
```
...
| `PLATFORM_WORKSPACE_ROOT` | `workspace_root` | `/tmp/cckit_workspaces` |
